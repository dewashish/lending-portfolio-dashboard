...
const TOOLTIP_CLASS = 'watchlist-trigger-tooltip';
...
interface Props {
  data: { category: string; exposure: number; count: number }[];
  onSliceClick?: (category: string) => void;
}

export function WatchlistTriggerDonut({ data, onSliceClick }: Props) {
  ...
  const ref = useD3Chart(
    (svg, width, height) => {
      ...
      const pie = d3.pie<{ category: string; exposure: number; count: number }>()
        .value(d => d.exposure)
        .sort(null);

      const arc = d3.arc<d3.PieArcDatum<{ category: string; exposure: number; count: number }>>()
      ...
      const labelArc = d3.arc<d3.PieArcDatum<{ category: string; exposure: number; count: number }>>()
  ...
  return (
    <ChartContainer title="Exposure by Trigger Category" subtitle="Watchlist exposure distribution" empty={totalExposure === 0}>
