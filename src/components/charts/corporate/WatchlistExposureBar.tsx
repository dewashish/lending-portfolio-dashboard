...
const TOOLTIP_CLASS = 'watchlist-exposure-tooltip';
...
export function WatchlistExposureBar({ data, totalExposure, onBarClick }: Props) {
  ...
  const ref = useD3Chart((svg, width, height) => {
    ...
    const margin = { top: 10, right: 110, bottom: 20, left: 140 };
    ...
    const x = d3.scaleLinear()
      .domain([0, d3.max(sorted, d => d.exposure) ?? 0])
      .range([0, w]);
    ...
    g.selectAll('.bar')
      ...
      .on('mouseover', function (_event, d) {
        ...
        tooltip.html(
          ...
          `<div><span style="color:${mutedColor}">Exposure:</span> <b>${formatCurrency(d.exposure)}</b></div>` +
          `<div><span style="color:${mutedColor}">Borrower Count:</span> <b>${d.count}</b></div>` +
          `<div><span style="color:${mutedColor}">Share of Total:</span> <b>${formatPercent(share)}</b></div>`
        ).style('opacity', '1');
    ...
    g.selectAll('.val')
      .data(sorted)
      .join('text')
      .attr('x', d => x(d.exposure) + 6)
      .attr('y', d => y(d.sector)! + y.bandwidth() / 2)
      .attr('dy', '0.35em')
      .attr('fill', d3Tokens.textMuted)
      .attr('font-size', '11px')
      .attr('font-family', 'IBM Plex Mono, monospace')
      .text(d => formatCurrency(d.exposure));
  ...
  return (
    <ChartContainer title="Exposure by Sector" subtitle="Watchlist exposure distribution" empty={!sorted.length}>
