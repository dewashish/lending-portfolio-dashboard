...
export function GroupTrendGrid({ consumerOverall, tradeSummary, corporateSummary }: Props) {
  const dpd30 = extractLatest(consumerOverall, '30+ Amt%');
  const dpd90 = extractLatest(consumerOverall, '90+ Amt%');
  const fpd = extractLatest(consumerOverall, 'FPD%');
  ...
  const cards = [
    ...
    {
      label: 'FPD%',
      value: fpd != null ? formatPercent(fpd) : '—',
      sparkline: extractSparkline(consumerOverall, 'FPD%'),
      trend: computeMoMTrend(consumerOverall, 'FPD%'),
      invertTrend: true,
      metricKey: 'fpd_pct',
      rawValue: fpd,
      color: undefined as string | undefined,
