  ...
  switch (intent) {
    case 'action':
      markdown =
        ...
        `The first lever moves the number fastest because it attacks the concentration I'm seeing in the underlying accounts — DSA-sourced unsecured files in Beltone Seven remain the common thread across the book this period. ` +
    ...
    case 'comparison':
      markdown =
        ...
        `Pre-policy cohorts and peer segments are aging inside their two-year corridor; the divergence concentrates in files scored 585–611, DSA-sourced, in Beltone Seven. Adjusted for mix, that policy band explains most of the gap you're looking at. ` +
        `LuloBank and Mirabank — which kept the old cut — are the clean control group.`;
      break;
    case 'segmentation':
      markdown =
        `**Segment attribution for ${subject}:** the excess is concentrated, not broad-based.\n\n` +
        `- **Product:** Seven Credit Card and unsecured Personal Loan carry ~70% of it\n` +
        `- **Channel:** DSA / mobile-agent sourcing over-indexes ~3× vs branch\n` +
        `- **Geography:** Cairo & Giza dominate; other regions are in corridor\n` +
    ...
    case 'data-quality':
      markdown =
        `**Data check on ${subject}:** ingestion watermarks show all subsidiary feeds current except **Samman Capital (47 days)** — any Samman-linked cell here should be re-verified before escalation.\n\n` +
        `Field-level DQ checks on the underlying table pass except for collateral revaluation dates. I'd refresh the Samman feed before treating this as a risk signal.`;
      break;
    case 'summary':
      markdown =
        `**${where} in brief:** performance is inside appetite except the early-stage credit cluster in Beltone Seven's unsecured digital book.\n\n` +
    ...
    default: // root-cause
      markdown =
        `**On ${subject}:** the movement traces to the post-March unsecured cohorts in Beltone Seven — the common root cause running through the book this period.\n\n` +
        `- The affected accounts are DSA-sourced, scored 585–611 at origination, concentrated in Cairo/Giza\n` +
